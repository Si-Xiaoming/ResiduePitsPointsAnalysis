...
@MODELS.register_module("Sonata-v1m1-Resolution")
class Sonata(PointModel):
    def __init__(
        ...
        self.density_contrast_weight = density_contrast_weight
    ...
    def forward(self, data_dict, return_point=False):
        ...
        with torch.no_grad():
            ...
            if self.density_contrast_weight > 0 and "sparse_coord" in data_dict:
                sparse_point = Point(
        ...
        if self.density_contrast_weight > 0 and "sparse_coord" in data_dict:
            # Student Forward (Sparse Input)
            sparse_point_ = self.student.backbone(sparse_point)
            sparse_point_ = self.up_cast(sparse_point_)
            
            # Pass through the SAME mask_head as the main task
            # This forces the semantic features to be invariant to density
            sparse_pred_feat = self.student.mask_head(sparse_point_.feat) # (N_sparse, C)

            # Correspondence Finding (Sparse -> Dense)
            with torch.no_grad():
                idx, dist = pointops.knn_query(
                    1, 
                    global_point_.coord.float(), global_point_.offset.int(), # Teacher (Dense)
                    sparse_point_.coord.float(), sparse_point_.offset.int()  # Student (Sparse)
                )
                # idx: (N_sparse, 1) index in dense point cloud
                target_idx = idx.long().squeeze()
