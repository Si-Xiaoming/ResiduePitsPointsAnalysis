...
def execuate_las(raw_path, grid_size, split, output_path, tile_size = 250.0, has_label=True, tile_orient=True):
    ...
    pipeline |= pdal.Filter.stats(dimensions="Red,Blue,Green")
    ...
    if has_label:
        ...
        pipeline |= pdal.Filter.range(limits="Classification[2:2], Classification[4:4], Classification[5:5], Classification[6:6], Classification[8:8], Classification[9:9], Classification[10:10]")
        
        pipeline |= pdal.Filter.assign(value=[
            f"Classification = 0 WHERE (Classification == 2 || Classification == 10)",
            f"Classification = 1 WHERE Classification == 4",
            f"Classification = 2 WHERE Classification == 6",
            f"Classification = 3 WHERE Classification == 5",
            f"Classification = 4 WHERE Classification == 8",
            f"Classification = 5 WHERE Classification == 9",
    ...
    # check the classification values: the unique values and the min/max
    unique_classes = np.unique(y)
    print(f"Unique classification values: {unique_classes}")
    print(f"Classification min: {y.min()}, max: {y.max()}")
